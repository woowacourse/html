...
- style : CSS 적용
- div : 레이아웃 나누기 위해 사용
- section : 시멘틱 태그이다 . 문서의 구역을 정의한다.
- article : 내용을 적는 공간이다.
- header : 페이지의 제목이나 서론이 주로 존재한다.
- footer : 페이지의 바닥글이며, 저작권, 연락처가 적힌다.
- p : 하나의 문단을 만들때 쓴다.
- img : 이미지를 삽입하기 위해 쓴다.
- table : 테이블 레이아웃을 위해 쓴다.
- a : 링크를 생성할수있으며 다른 웹페이지나 현재페이지에서 특정 경로로 이동할수 있다.
- strong : 인라인 태그이며 특정 구문을 강조할때 사용한다.